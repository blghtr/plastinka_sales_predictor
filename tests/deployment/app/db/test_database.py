...
def test_execute_query_error(in_memory_db):
    ...
    # The error should contain information about the failed query
...
def test_create_or_get_config(in_memory_db, sample_config, create_training_params_fn):
    ...
    same_config_id = create_or_get_config(
        sample_config, connection=in_memory_db._connection
    )
    ...
    new_config_id = create_or_get_config(
        new_config_data, is_active=True, connection=in_memory_db._connection
    )
...
def test_get_active_config(in_memory_db, sample_config):
    ...
    execute_query("UPDATE configs SET is_active = 0", connection=in_memory_db._connection)
    assert get_active_config(in_memory_db._connection) is None
...
def test_set_config_active(in_memory_db, sample_config, create_training_params_fn):
    ...
    config_id1 = create_or_get_config(
        sample_config, is_active=False, connection=in_memory_db._connection
    )
    ...
    config_id2 = create_or_get_config(
        new_config_data, is_active=False, connection=in_memory_db._connection
    )
    ...
    success = set_config_active(config_id1, connection=in_memory_db._connection)
    assert success

    # Verify config_id1 is active
    active_config = get_active_config(in_memory_db._connection)
    ...
    success = set_config_active(config_id2, deactivate_others=False, connection=in_memory_db._connection)
    assert success
    active_config = get_active_config(in_memory_db._connection)
    ...
    success = set_config_active(config_id2, connection=in_memory_db._connection)
    assert success
    ...
    non_existent_id = str(uuid.uuid4())
    success = set_config_active(non_existent_id, connection=in_memory_db._connection)
    assert not success
...
def test_get_effective_config_active_and_best(in_memory_db, sample_config):
    ...
    job_id = create_job("training", {}, connection=in_memory_db._connection)
    ...
    best_metric_config_id = create_or_get_config(
        best_metric_config_data, connection=in_memory_db._connection
    )
    ...
    active_config_id = create_or_get_config(
        active_config_data, is_active=True, connection=in_memory_db._connection
    )

    effective_config = get_effective_config(
        mock_settings, logger=MagicMock(), connection=in_memory_db._connection
    )
...
def test_get_effective_config_only_best(in_memory_db, sample_config):
    ...
    best_metric_config_id = create_or_get_config(
        best_metric_config_data, is_active=False, connection=in_memory_db._connection
    )
...
def test_get_effective_config_no_config(in_memory_db):
    ...
    execute_query("DELETE FROM configs", connection=in_memory_db._connection)
    execute_query("DELETE FROM training_results", connection=in_memory_db._connection)
...
def test_get_top_tuning_results_empty(in_memory_db):
    ...
    results = get_tuning_results(in_memory_db._connection) # Using get_tuning_results
    assert results == []
...
def test_get_top_tuning_results_basic_sorting(in_memory_db, tuning_results_data):
    ...
    results = get_tuning_results(metric_name="val_MIC", higher_is_better=False, connection=in_memory_db._connection)
...
def test_get_top_tuning_results_limit(in_memory_db, tuning_results_data):
    ...
    results = get_tuning_results(metric_name="val_MIC", higher_is_better=False, limit=2, connection=in_memory_db._connection)
    assert results is not None, "get_tuning_results should not return None"
...
def test_get_top_tuning_results_threshold(in_memory_db, tuning_results_data):
    ...
    # results = get_tuning_results("val_MIC", higher_is_better=False, threshold=12.0, connection=in_memory_db._connection)
    # # mape <= 12.0: 5.0, 10.0
...
def test_get_top_tuning_results_invalid_metric(in_memory_db):
    ...
    with pytest.raises(ValueError, match="Invalid metric name"):
        get_tuning_results(metric_name="not_a_metric", higher_is_better=False, connection=in_memory_db._connection)
...
def test_get_top_configs_include_active(configs_with_metrics, in_memory_db):
    ...
    # Используем DAL для вызова стандартизированной функции
    dal = in_memory_db
    results = dal.get_top_configs(limit=2, metric_name="val_MIC", higher_is_better=False, include_active=True)
...
def test_get_top_configs_sorting_by_metric(configs_with_metrics, in_memory_db):
    ...
    dal = in_memory_db
    results = dal.get_top_configs(limit=3, metric_name="val_MIC", higher_is_better=False, include_active=False)
...
def test_get_top_configs_limit(configs_with_metrics, in_memory_db):
    ...
    _, cfg_ids = configs_with_metrics
    dal = in_memory_db
    results = dal.get_top_configs(limit=2, metric_name="val_MIC", higher_is_better=False, include_active=False)
    assert len(results) == 2
...
def test_get_top_configs_include_active_false(configs_with_metrics, in_memory_db):
    ...
    active_id, cfg_ids = configs_with_metrics
    dal = in_memory_db
    results = dal.get_top_configs(limit=5, metric_name="val_MIC", higher_is_better=False, include_active=False)
...
def test_get_top_configs_invalid_metric(configs_with_metrics, in_memory_db):
    ...
    with pytest.raises(ValueError, match="Invalid metric_name"):
        dal.get_top_configs(limit=2, metric_name="not_a_metric")
...
def test_get_top_configs_all_metrics_null(in_memory_db):
    ...
    for i in range(3):
        ...
        cursor.execute("INSERT INTO configs (config_id, config, is_active, created_at) VALUES (?, ?, 0, ?)", (cfg_id, json.dumps(cfg), datetime.now().isoformat()))
    conn.commit()
    results = dal.get_top_configs(limit=3, metric_name="val_MIC", higher_is_better=False)
...
def test_get_best_config_by_metric_integration(in_memory_db, sample_config, monkeypatch):
    ...
    dal.execute_raw_query("INSERT INTO models (model_id, job_id, model_path, created_at) VALUES (?, ?, ?, ?)", (model_id_base, job_id_base, "/path/to/model.onnx", datetime.now().isoformat()))
    ...
    config_id_2 = dal.create_or_get_config(config_data_2)
    job_id_2 = dal.create_job("tuning", {})
    dal.create_tuning_result(job_id_2, config_id_2, {"val_MIC": 0.05}, 150)
    ...
    config_id_4 = dal.create_or_get_config(config_data_4)
    job_id_4 = dal.create_job("tuning", {})
    dal.create_tuning_result(job_id_4, config_id_4, {"val_MIC": 0.15}, 180)

    # Test with higher_is_better=False (lower val_MIC is better)
    best_config = dal.get_best_config_by_metric("val_MIC", higher_is_better=False)
    ...
    config_id_6 = dal.create_or_get_config(config_data_6)
    job_id_6 = dal.create_job("tuning", {})
    dal.create_tuning_result(job_id_6, config_id_6, {"val_MIC": 0.8}, 220)

    best_config_higher = dal.get_best_config_by_metric("val_MIC", higher_is_better=True)
    ...
    with pytest.raises(ValueError, match="Invalid metric_name"):
        dal.get_best_config_by_metric("non_existent_metric")
...
class TestDeterminePredictionMonth:
    ...
    def test_invalid_date_range(self, db_with_sales_data):
        ...
        dal = db_with_sales_data
        result = dal.adjust_dataset_boundaries(start, end)
...
class TestMultiIndexBatch:
    def test_batch_insert_all_new(self, in_memory_db):
        ...
        id_map = get_or_create_multiindex_ids_batch(tuples, connection=conn)
    ...
    def test_batch_mixed_new_and_existing(self, in_memory_db):
        ...
        from deployment.app.db.database import get_or_create_multiindex_ids_batch
        id_map = get_or_create_multiindex_ids_batch([existing_tuple, new_tuple], connection=conn)
