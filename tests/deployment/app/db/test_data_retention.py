...
class TestDataRetention(unittest.TestCase):
    def setUp(self):
        ...
        self.settings_patch = patch('deployment.app.db.data_retention.settings')
        ...
        self.mock_settings.data_retention = mock_retention_settings
        self.mock_settings.default_metric = "val_MIC"
        self.mock_settings.default_metric_higher_is_better = True
    ...
    def _create_test_models(self):
        ...
        self.cursor.execute(
            ...
            (parameter_set_id, parameters, created_at, is_active)
            VALUES (?, ?, ?, ?)
            """,
            ("param_set1", json.dumps({"epochs": 100}), 
             now.strftime('%Y-%m-%d %H:%M:%S'), 1)
        )
        
        # Associate models with parameter set and add metrics
        self.cursor.executemany(
            """
