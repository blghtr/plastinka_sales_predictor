...
def test_execute_many_transaction(isolated_db_session):
    ...
    execute_many(
        "INSERT INTO test_batch (value) VALUES (?)", params_list, connection=conn
    )
...
def test_create_job_transaction_safety(isolated_db_session):
    ...
    with get_db_connection(db_path) as conn_success:
        job_id_success = create_job(
            "success_test", {"param": "value"}, connection=conn_success
        )
    ...
    try:
        with get_db_connection(db_path) as conn_fail:
            job_id_fail = create_job(
                "fail_test", {"param": "value"}, connection=conn_fail
            )
        ...
        try:
            with get_db_connection(db_path) as conn:
                ...
                result = conn.execute(
                    ...
                    (f"read_test_{thread_id}",),
                ).fetchone()
                if result:
                    job_id = (
                        result[0] if isinstance(result, tuple) else result["job_id"]
                    )
    ...
    def worker(db_path, thread_id, success_list):
        ...
        try:
            ...
            with get_db_connection(db_path) as conn:
                ...
                for i in range(5):
                    conn.execute(
                        "INSERT INTO concurrent_test VALUES (?, ?)", (i, thread_id)
                    )
                conn.commit()
...
def test_connection_isolation(isolated_db_session):
    ...
    try:
        ...
        value = (
            res2_after_commit[0]
            if isinstance(res2_after_commit, tuple)
            else res2_after_commit["id"]
        )
        assert value == 1
    finally:
        conn1.close()
...
def test_transaction_with_direct_conn_and_db_functions(isolated_db_session):
    ...
    with get_db_connection(db_path) as conn_verify:
        ...
        res = conn_verify.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='complex_trans'"
        ).fetchone()
