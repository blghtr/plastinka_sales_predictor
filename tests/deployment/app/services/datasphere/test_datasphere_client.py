...
# ========== AUTHENTICATION METHOD TESTS REMOVED ==========
# All auth_method tests removed after migration to OAuth-only authentication.
# yc_profile and auth_method parameters removed from DataSphereClient constructor.
# Only oauth_token is now supported.

# Test initialization
def test_init_success():
    """Test successful client initialization with oauth_token-only API."""
    with patch("deployment.datasphere.client.DatasphereClient") as mock_client_class:
        ...
        # Check that DatasphereClient was initialized with oauth_token only (yc_profile always None)
...
def test_init_failure():
    ...
    with patch("deployment.datasphere.client.DatasphereClient") as mock_client_class:
        ...
        with pytest.raises(
            DataSphereClientError,
            match="Failed to initialize official DataSphere client",
        ):
...
@patch(
    ...
    read_data="name: test-job\ncmd: python test.py\noutputs:\n  - output.txt: OUTPUT",
)
def test_submit_job_success(
    mock_open_file,
    mock_check_limits,
    mock_prepare_local_modules,
    mock_prepare_inputs,
    mock_define_py_env,
    mock_parse_config,
    client,
    mock_datasphere_client,
):
...
def test_submit_job_create_failure(
    mock_open_file,
    mock_check_limits,
    mock_prepare_local_modules,
    mock_prepare_inputs,
...
def test_download_job_results_success(client, mock_datasphere_client):
    ...
    client.download_job_results(
        TEST_JOB_ID, "/tmp/output", with_logs=True, with_diagnostics=True
    )
