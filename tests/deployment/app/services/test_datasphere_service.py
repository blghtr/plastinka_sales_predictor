...
class TestDataSphereNewArchitecture:
    ...
    def test_datasphere_settings_mock(self, mock_datasphere_env):
        ...
        # Verify DataSphere api_client config
    ...
    def test_datasphere_client_mock(self, mock_datasphere_env):
        """Test that DataSphere api_client is properly mocked."""
        api_client = mock_datasphere_env["api_client"]

        # Test job submission
        job_id = api_client.submit_job("fake_config_path", "fake_work_dir")
        assert job_id == "ds-job-default-new-arch"

        # Test status check
        status = api_client.get_job_status("test-job-id")
        ...
        api_client.download_job_results("test-ds-job", results_dir)
    ...
    async def test_save_model_with_new_architecture(
        self, mock_datasphere_env, temp_workspace, monkeypatch
    ):
        ...
        # Insert job with the required fields using DAL
    ...
    def test_mock_reset_functionality(self, mock_datasphere_env):
        ...
        api_client.submit_job.return_value = "modified-job-id"
        assert api_client.submit_job("test") == "modified-job-id"
...
class TestDataSphereServiceTuning:
    ...
    def test_process_tuning_job_results_success(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_or_get_config", mock_create_or_get_config)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_tuning_result", mock_create_tuning_result)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.update_job_status", mock_update_job_status)
    ...
    def test_process_tuning_job_results_missing_file(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.update_job_status", mock_update_status)
    ...
    def test_process_tuning_job_results_database_error(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        mock_update_status = MagicMock()
        mock_get_db_connection = MagicMock()
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_or_get_config", mock_create_config)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.update_job_status", mock_update_status)
    ...
    async def test_run_job_tune_branching(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        NB: execute_query больше не вызывается напрямую, source обновляется через create_or_get_config(..., source="tuning")."""
        from deployment.app.services.datasphere_service import JOB_TYPE_TUNE, run_job
        ...
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_or_get_config", mock_create_config)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_tuning_result", mock_create_tuning)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.update_job_status", mock_update_status)

        # Mock _prepare_job_datasets to actually create required files
        async def mock_prepare_datasets(job_id, config, dal, start_date_for_dataset=None, end_date_for_dataset=None, output_dir=None, **kwargs):
        ...
        mock_init_client.return_value = mock_datasphere_env["api_client"]
    ...
    async def test_run_job_train_branching(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        from deployment.app.services.datasphere_service import JOB_TYPE_TRAIN, run_job
...
class TestDataSphereServiceTuningIntegration:
    ...
    async def test_complete_tuning_workflow(self, temp_workspace, mock_datasphere_env, monkeypatch):
        ...
        monkeypatch.setattr(mock_datasphere_env["api_client"], "download_job_results", mock_tuning_download)
        ...
        monkeypatch.setattr("deployment.app.services.datasphere_service._prepare_job_datasets", mock_prepare)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_or_get_config", mock_create_config_db)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.create_tuning_result", mock_create_tuning_db)
        monkeypatch.setattr("deployment.app.db.data_access_layer.DataAccessLayer.update_job_status", mock_update_status)
        mock_get_db_connection = MagicMock()
        ...
        # Mock the job status to return completed immediately
        mock_datasphere_env["api_client"].dal.get_job_status.return_value = "COMPLETED"
...
async def test_download_datasphere_job_results_unified(mock_datasphere_env, monkeypatch):
    # Arrange
    mock_ds_client = mock_datasphere_env["api_client"]
    ...
    try:
        ...
        # Mock download_job_results (now part of mock_datasphere_env["api_client"]) to not do anything
