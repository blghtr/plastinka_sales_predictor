...
def load_sample_data():
    ...
    sample_path = (
        ISOLATED_TESTS_BASE_DIR / "process_raw" / "inputs" / "stock_df_raw.pkl"
    )
...
class TestCategorizePricesCore:
    ...
    def test_categorize_prices_default_quantiles(self):
        ...
        assert categorized_ratio >= 0.8, (
            f"Too few prices categorized: {categorized_ratio:.2f}"
        )
    ...
    def test_categorize_prices_custom_quantiles(self):
        ...
        first_cat_prices = result_df[result_df["Ценовая категория"] == sorted_cats[0]][
            "Цена, руб."
        ]
        second_cat_prices = result_df[result_df["Ценовая категория"] == sorted_cats[1]][
            "Цена, руб."
        ]

        assert (first_cat_prices <= median_price).all(), (
            "First category should contain lower prices"
        )
        assert (second_cat_prices > median_price).all(), (
            "Second category should contain higher prices"
        )
    ...
    def test_categorize_prices_with_predefined_bins(self):
        ...
        np.testing.assert_array_equal(
            initial_bins, returned_bins, "Returned bins should match input bins"
        )

        # Verify consistent categorization
        unique_categories = result_df["Ценовая категория"].nunique()
        assert unique_categories == 2, (
            f"Expected 2 categories with predefined bins, got {unique_categories}"
        )
...
class TestIntegration:
    ...
    def test_constants_defined(self):
        ...
        # Assert
        assert hasattr(test_module, "ISOLATED_TESTS_BASE_DIR"), (
            "ISOLATED_TESTS_BASE_DIR should be defined"
        )
        assert isinstance(test_module.ISOLATED_TESTS_BASE_DIR, Path), (
            "ISOLATED_TESTS_BASE_DIR should be a Path object"
        )
    ...
    def test_utility_functions_available(self):
        ...
        # Assert
        assert hasattr(test_module, "load_sample_data"), (
            "load_sample_data function should be available"
        )
        assert callable(test_module.load_sample_data), (
            "load_sample_data should be callable"
        )
...
def test_categorize_prices_quantiles(mock_load_data):
    ...
    assert "Ценовая категория" in result_df.columns, (
        "Column 'Ценовая категория' not found in result"
    )

    unique_categories = result_df["Ценовая категория"].unique()
    assert len(unique_categories) <= 7, (
        f"Too many unique categories: {len(unique_categories)} > 7"
    )
    ...
    assert null_count <= 2, (
        f"Found {null_count} null values in 'Ценовая категория' column (max allowed: 2)"
    )
...
def test_categorize_prices_expected_bins():
    ...
    assert first_category_counts == 4, (
        f"Expected 4 items in first category, got {first_category_counts}"
    )
    assert second_category_counts == 3, (
        f"Expected 3 items in second category, got {second_category_counts}"
    )
