...
def _load_artifact(file_path: Path):
    ...
    if file_path.suffix == ".json":
        ...
        pytest.fail(
            f"Unsupported artifact file extension: {file_path.suffix} for file {file_path}. Only .json and .pkl are supported."
        )


def _load_isolated_input_artifact(func_name: str, artifact_name: str):
    return _load_artifact(
        ISOLATED_TESTS_BASE_DIR / func_name / "inputs" / artifact_name
    )


def _load_isolated_output_artifact(func_name: str, artifact_name: str):
    return _load_artifact(
        ISOLATED_TESTS_BASE_DIR / func_name / "outputs" / artifact_name
    )
...
def _run_dataframe_test(
    actual_df: pd.DataFrame, expected_artifact_path_or_df, stage_name: str
):
...
def _run_dataframe_tuple_test(
    actual_tuple: tuple, expected_artifact_path_or_tuple, stage_name: str
):
    ...
    try:
        ...
        pytest.fail(
            f"DataFrame comparison failed for stage '{stage_name}' (DataFrame part):\n{e}"
        )

    # Compare bins part
    if not isinstance(actual_bins, type(expected_bins)):
        pytest.fail(
            f"Bins type mismatch for stage '{stage_name}': Actual type {type(actual_bins)}, Expected type {type(expected_bins)}"
        )
    ...
    if isinstance(actual_bins, np.ndarray) and isinstance(expected_bins, np.ndarray):
        if not np.array_equal(actual_bins, expected_bins):
            pytest.fail(
                f"Bins comparison failed for stage '{stage_name}':\nActual: {actual_bins}\nExpected: {expected_bins}"
            )
...
def _run_dict_of_dataframes_test(
    actual_dict_df: dict, expected_artifacts_map: dict, stage_name: str
):
    ...
    for key, expected_path in expected_artifacts_map.items():
        ...
        _run_dataframe_test(
            actual_dict_df[key], expected_path, f"{stage_name} - key: {key}"
        )


def _run_dataset_test(
    actual_dataset_dict: dict, expected_artifact_path_or_dict, stage_name: str
):
    ...
    if comparison_results.get("length_differences"):
        diff_messages.append(
            f"Length differences: {comparison_results['length_differences']}"
        )
    if comparison_results.get("type_differences"):
        diff_messages.append(
            f"Type differences: {comparison_results['type_differences']}"
        )
    if comparison_results.get("value_differences"):
        diff_messages.append(
            f"Value differences: {comparison_results['value_differences']}"
        )
    if comparison_results.get("sample_differences"):
        diff_messages.append(
            f"Sample differences: {comparison_results['sample_differences']}"
        )

    if diff_messages:
        pytest.fail(
            f"Dataset comparison failed for stage '{stage_name}':\n"
            + "\n".join(diff_messages)
...
class TestDataProcessingCore:
    ...
    def test_process_raw_stock_success(self):
        ...
        expected_processed_stock_path = (
            ISOLATED_TESTS_BASE_DIR / "process_raw" / "outputs" / "processed_stock.pkl"
        )
    ...
    def test_process_raw_sales_success(self):
        ...
        expected_processed_sales_path = (
            ISOLATED_TESTS_BASE_DIR / "process_raw" / "outputs" / "processed_sales.pkl"
        )
        ...
        # Assert
        _run_dataframe_test(
            actual_processed_sales, expected_processed_sales_path, "process_raw (sales)"
        )
    ...
    def test_process_raw_with_mocked_dependencies(
        self,
        mock_categorize_prices,
        mock_categorize_dates,
        mock_validate_styles,
        ...
        test_df_with_price_category["Ценовая категория"] = pd.Categorical(
            ["Low", "Medium", "High"]
        )
        mock_categorize_prices.return_value = (
            test_df_with_price_category,
...
class TestDataFiltering:
    ...
    def filter_by_date_stock_input_df(self):
        ...
        return _load_isolated_input_artifact(
            "filter_by_date", "processed_stock_input.pkl"
        )
    ...
    def test_filter_by_date_stock_before_cutoff(self, filter_by_date_stock_input_df):
        ...
        expected_path = (
            ...
            / "filter_by_date"
            / "outputs"
            / "stock_filtered_before_cutoff.pkl"
        )

        # Act
        actual_filtered_stock = filter_by_date(
            filter_by_date_stock_input_df.copy(), CUTOFF_DATE, cut_before=False
        )

        # Assert
        _run_dataframe_test(
            actual_filtered_stock,
            expected_path,
    ...
    def test_filter_by_date_stock_after_cutoff(self, filter_by_date_stock_input_df):
        ...
        expected_path = (
            ...
            / "filter_by_date"
            / "outputs"
            / "stock_filtered_after_cutoff.pkl"
        )

        # Act
        actual_filtered_stock = filter_by_date(
            filter_by_date_stock_input_df.copy(), CUTOFF_DATE, cut_before=True
        )
    ...
    def test_filter_by_date_sales_before_cutoff(self, filter_by_date_sales_input_df):
        ...
        expected_path = (
            ...
            / "filter_by_date"
            / "outputs"
            / "sales_filtered_before_cutoff.pkl"
        )

        # Act
        actual_filtered_sales = filter_by_date(
            filter_by_date_sales_input_df.copy(), CUTOFF_DATE, cut_before=False
        )

        # Assert
        _run_dataframe_test(
            actual_filtered_sales,
            expected_path,
    ...
    def test_filter_by_date_sales_after_cutoff(self, filter_by_date_sales_input_df):
        ...
        expected_path = (
            ...
            / "filter_by_date"
            / "outputs"
            / "sales_filtered_after_cutoff.pkl"
        )

        # Act
        actual_filtered_sales = filter_by_date(
            filter_by_date_sales_input_df.copy(), CUTOFF_DATE, cut_before=True
        )

        # Assert
        _run_dataframe_test(
            actual_filtered_sales, expected_path, "filter_by_date (sales, after cutoff)"
        )
...
class TestDataPreprocessing:
    ...
    def test_get_preprocessed_df_stock_success(self):
        ...
        # Arrange
        stock_df_raw = _load_isolated_input_artifact(
            "get_preprocessed_df", "stock_df_raw.pkl"
        )
        expected_path = (
            ISOLATED_TESTS_BASE_DIR
            / "get_preprocessed_df"
            / "outputs"
            / "preprocessed_stock_with_bins.pkl"
        )
        ...
        # Act
        actual_result = get_preprocessed_df(
            stock_df_raw.copy(), all_keys, count_items_stock
        )

        # Assert
        _run_dataframe_tuple_test(
    ...
    def test_get_preprocessed_df_sales_success(self):
        ...
        # Arrange
        sales_df_raw = _load_isolated_input_artifact(
            "get_preprocessed_df", "sales_df_raw.pkl"
        )
        stock_preprocessed_tuple = _load_isolated_output_artifact(
            "get_preprocessed_df", "preprocessed_stock_with_bins.pkl"
        )
        ...
        expected_path = (
            ...
            / "get_preprocessed_df"
            / "outputs"
            / "preprocessed_sales_with_bins.pkl"
        )
        ...
        # Act
        actual_result = get_preprocessed_df(
            sales_df_raw.copy(), all_keys, process_movements_sales, bins=stock_bins
        )

        # Assert
        _run_dataframe_tuple_test(
            actual_result, expected_path, "get_preprocessed_df (sales with stock bins)"
        )

    @patch("plastinka_sales_predictor.data_preparation.categorize_prices")
    def test_get_preprocessed_df_with_mocked_categorize_prices(
        self, mock_categorize_prices
    ):
...
class TestPipelineIntegration:
    ...
    def test_process_data_success(self):
        ...
        # Arrange
        input_parameters = _load_isolated_input_artifact(
            "process_data", "input_parameters.pkl"
        )
        expected_outputs_map = {
            key: ISOLATED_TESTS_BASE_DIR
            / "process_data"
            / "outputs"
            / f"feature_{key}.pkl"
        ...
        # Assert
        _run_dict_of_dataframes_test(
            actual_features_dict, expected_outputs_map, "process_data"
        )
...
class TestStockFeatures:
    ...
    def test_get_stock_features_success(self):
        ...
        # Arrange
        stock_data = _load_isolated_input_artifact(
            "get_stock_features", "stock_data.pkl"
        )
        change_data = _load_isolated_input_artifact(
            "get_stock_features", "change_data.pkl"
        )
        expected_path = (
            ISOLATED_TESTS_BASE_DIR
            / "get_stock_features"
    ...
    def test_get_stock_features_with_mocked_ensure_monthly_regular_index(self):
        ...
        date_index = pd.date_range(
            "2022-01-01", periods=3, freq="ME"
        )  # Используем ME вместо M

        test_stock = pd.DataFrame(
            {"feature1": [1, 2, 3], "feature2": [4, 5, 6]}, index=date_index
        )
        ...
        test_movements = pd.DataFrame({"change": [7, 8, 9]}, index=multi_index)
...
class TestSalesPivoting:
    ...
    def test_get_monthly_sales_pivot_success(self):
        ...
        # Arrange
        sales_data = _load_isolated_input_artifact(
            "get_monthly_sales_pivot", "sales_data.pkl"
        )
        expected_path = (
            ISOLATED_TESTS_BASE_DIR
            / "get_monthly_sales_pivot"
    ...
    def test_get_monthly_sales_pivot_with_mocked_transform_months(self):
        ...
        test_df = pd.DataFrame(
            {
                "Дата продажи": pd.to_datetime(
                    ["2022-01-01", "2022-02-01", "2022-01-15", "2022-02-15"]
                ),
        ...
        test_df = (
            test_df.groupby(["Штрихкод", "Исполнитель", "Альбом", "Дата продажи"])
            .agg({"count": "sum"})
            .reset_index()
        )
        test_df["_date"] = test_df[
            "Дата продажи"
        ]  # Добавляем _date колонку как ожидается в коде
        test_df["sales"] = test_df[
            "count"
        ]  # Добавляем sales колонку как ожидается в get_monthly_sales_pivot
...
class TestTrainingDataset:
    ...
    def test_plastinka_training_ts_dataset_success(self):
        ...
        # Arrange
        stock_features = _load_isolated_input_artifact(
            "training_dataset", "stock_features.pkl"
        )
        monthly_sales_pivot = _load_isolated_input_artifact(
            "training_dataset", "monthly_sales_pivot.pkl"
        )
        expected_path = (
            ISOLATED_TESTS_BASE_DIR
            / "training_dataset"
            / "outputs"
            / "full_dataset_to_dict.pkl"
        )
        ...
        _run_dataset_test(
            actual_dataset_dict, expected_path, "PlastinkaTrainingTSDataset"
        )
    ...
    def test_plastinka_training_ts_dataset_with_save(self, mock_dill_dump):
        ...
        monthly_sales = pd.DataFrame(
            index=date_index,
            columns=pd.MultiIndex.from_tuples([("item1",), ("item2",)]),
        )
        # Заполняем данными
    ...
    def test_plastinka_training_ts_dataset_from_dill(
        self, mock_file_open, mock_dill_load
    ):
...
class TestTransformers:
    ...
    def test_multi_column_label_binarizer_fit_transform(self):
        ...
        assert isinstance(result, pd.DataFrame)
        assert (
            result.shape[1] > test_data.shape[1]
        )  # Should have more columns after binarization

    def test_global_log_min_max_scaler_fit_transform(self):
        """Test GlobalLogMinMaxScaler fit and transform operations."""
        # Arrange
        scaler = GlobalLogMinMaxScaler()
        test_data = pd.DataFrame({"values": [1e-6, 0.5, 1.5, 2.0]})
...
class TestIntegration:
    ...
    def test_end_to_end_pipeline_integration(self, mock_read_csv, mock_read_excel):
        ...
        # Act
        result_dict = process_data("mock_stock.xlsx", "mock_sales.csv", "2023-01-01")
